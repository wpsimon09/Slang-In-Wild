...
<section class="f-full bg-zinc-100 font-chivo min-h-screen flex flex-col items-center">
        ...
        <div class="w-full ">
                <div class="w-full flex flex-row items-center lg:justify-between">
                    ...
                    <div class="flex w-1/3 lg:justify-end h-10  mt-4 mr-4">
                        <button onclick={openSubmitForm} class="text-white text-xs text-center h-full lg:text-lg duration-200 lg:w-40 w-full hover:text-white hover:cursor-pointer hover:bg-teal-700 p-2 rounded-lg shadow-xl bg-amber-600" >Submit project</button>
                    </div>
                </div>
                
                <div class="w-full flex-row felx items-center">
                    <div class="w-full justify-between px-4 flex mt-4 flex-row">
                        <h2 class="w-full text-left font-bold mt-2">Filter by tags:</h2>
                        <button onclick={clearTags} class="h-8 w-8 bg-amber-600 p-1  rounded-full hover:bg-teal-700 hover:rotate-180 duration-200 cursor-pointer">
                            <img alt="reset filters" class="h-full w-full" src="svg/reset-filter-2.svg"/>
                        </button>
                    </div>
    ...
    <div class="md:w-[70%] lg:w-[60%] flex-1 flex flex-row items-center w-full mt-4 overflow-hidden">
    {#if FilteredProjects.length === 0}
        <div transition:scale={{duration: 220,  easing: expoInOut}} class="flex-1 w-full h-full flex  flex-col items-center justify-center text-zinc-400 text-xl">
            No projects found.
            <button onclick={clearTagsAndSearchText} class="text-zinc-400 text-xl hover:text-2xl duration-200 hover:cursor-pointer underline">
